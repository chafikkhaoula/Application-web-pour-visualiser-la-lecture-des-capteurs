...
function epochToJsDate(epochTime){
  ...
  const soilElement = document.getElementById("sol");
    ...
    if (user) {
      ...
      chartRef.on('value', snapshot =>{
        ...
        dbRef.orderByKey().limitToLast(chartRange).on('child_added', snapshot =>{
          var jsonData = snapshot.toJSON(); // example: {temperature: 25.02, humidity: 50.20, humidityofsoil: 1008.48, timestamp:1641317355}
          // Save values on variables
          var temperature = jsonData.temperature;
          var humidity = jsonData.humidity;
          var humidityofsoil= jsonData.humidityofsoil;
          var timestamp = jsonData.timestamp;
          // Plot the values on the charts
          plotValues(chartT, timestamp, temperature);
          plotValues(chartH, timestamp, humidity);
          plotValues(chartP, timestamp, humidityofsoil);
        });
      });
      ...
      dbRef.orderByKey().limitToLast(1).on('child_added', snapshot =>{
        ...
        soilElement.innerHTML = humidityofsoil;
      ...
      // append readings to table (after click More results... button)
          ...
          if (snapshot.exists()) {
            ...
            reversedList.forEach(element =>{
              if (firstTime){ // ignore first reading (it's already on the table from the previous query)
                ...
                var humidityofsoil = element.humidityofsoil;
